...
public class StarvingThread {
    ...
    public static void main(String[] args) {
        ...
        Callable<Boolean> thread = () -> {
            ...
            int threadNumber = Integer.parseInt(threadName.replaceAll(".*-[1-9]*-.*-", ""));
            boolean keepGoing = true;

            while (keepGoing) {
                lock.lock();
                try {
                    System.out.printf("%d acquired the lock.%n", threadNumber);
                    Thread.sleep(1000);
                } catch (InterruptedException e) {
                    System.out.printf("Shutting down %d%n", threadNumber);
