...
public class Student {
    ...
    public int getMonthsSinceLastActivity(String courseCode) {
        CourseEngagement info = engagementMap.get(courseCode);
        return info == null ? 0 : info.getMonthsSinceLastActivity();
    }

    public int getMonthsSinceLastActivity() {
        int inactiveMonths = (LocalDate.now().getYear() - 2014) * 12;
        for (String key : engagementMap.keySet()) {
            inactiveMonths = Math.min(inactiveMonths, getMonthsSinceLastActivity(key));
        }
    ...
    public double getPercentComplete(String courseCode) {
        ...
        return (info == null) ? 0.0 : info.getPercentComplete();
    }
    ...
    private static String getRandomValues(String... data) {
    ...
    public static Student getRandomStudent(Course... courses) {
        ...
        Student student = new Student(
            getRandomValues("USA", "IND", "CHN", "CAN", "AUS", "GBR", "FRA", "RUS", "JPN"),
            random.nextInt(2015, maxYear),
            random.nextInt(16, 31),
            getRandomValues("M", "F", "O"),
            random.nextBoolean(),
            randomCourses
        );
